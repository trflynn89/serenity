...
JS_DEFINE_NATIVE_FUNCTION(CollatorPrototype::resolved_options)
{
    ...
    auto* collator = typed_this_object(global_object);
    if (vm.exception())
        return {};
    ...
    options->create_data_property_or_throw(vm.names.locale, js_string(vm, collator->locale()));
    options->create_data_property_or_throw(vm.names.usage, js_string(vm, collator->usage_string()));
    options->create_data_property_or_throw(vm.names.sensitivity, js_string(vm, collator->sensitivity_string()));
    options->create_data_property_or_throw(vm.names.ignorePunctuation, Value(collator->ignore_punctuation()));
    options->create_data_property_or_throw(vm.names.collation, js_string(vm, collator->collation()));
    options->create_data_property_or_throw(vm.names.numeric, Value(collator->numeric()));
    if (collator->has_case_first())
        options->create_data_property_or_throw(vm.names.caseFirst, js_string(vm, collator->case_first()));
