...
void OutOfProcessWebView::enqueue_native_event(Web::DragEvent::Type type, GUI::DropEvent const& event)
{
    ...
    auto button = static_cast<Web::UIEvents::MouseButton>(to_underlying(event.button()));
    auto buttons = static_cast<Web::UIEvents::MouseButton>(event.buttons());
    auto modifiers = static_cast<Web::UIEvents::KeyModifier>(event.modifiers());
